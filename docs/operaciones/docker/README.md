...
> `Kong` fue la solucion de API Gateway implementada y evaluada en la primera fase del proyecto.
> La direccion actual del proyecto es migrar esa capa a `WSO2 API Manager`, manteniendo `Nginx Edge` como reverse proxy.
> Por eso esta documentacion puede mezclar referencias historicas de `Kong` con la arquitectura objetivo basada en `WSO2`.
...
| **Gateway** | `docker-compose-gateway.yml` | Nginx, Kong (fase 1) / WSO2 APIM (objetivo) | API Gateway y reverse proxy | [📖 README-gateway.md](readme_gateway.md) |
...
| docker-compose-base.yml          | kong-db | postgres | 15 (legado de fase 1) |
...
| docker-compose-gateway.yml       | kong | kong | 3.6 (legado de fase 1) |
